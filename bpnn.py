...
load_input = np.load("nn_input2.npy")[0:2700]
load_class = np.load("nn_class2.npy")[0:2700]
print("Eğitim Veri Sayısı : " +str(len(load_input)))
load_input_weight = np.load("nn_weight/input_weight.npy")
load_h1_weight = np.load("nn_weight/h1_weight.npy")
load_h1_bias = np.load("nn_weight/h1_bias.npy")
load_output_bias = np.load("nn_weight/output_bias.npy")

target_1_count = 0
target_0_count = 0
for i in range(len(load_class)):
    if int(load_class[i]) == 1:
        target_1_count = target_1_count + 1
    elif int(load_class[i]) == 0:
